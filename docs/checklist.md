...
- 第 1–2 天：第 4 章（基本群）——先把 $\pi_1(S^1)$ 证明默写一遍，
再做 van Kampen 三个应用；
- 第 3 天：第 8 章（$S^n$ 同调 + 度）——M–V 算 $S^n$、Brouwer、毛球；
- 第 4 天：第 5 章（复迭空间）——提升定理群 + 三个计算（$\mathbb{R}P^{n}$、周期商、叶数）；
- 第 5 天：第 7、9 章（同调）——链复形演算 + 相对同调长正合列；
- 第 6 天：第 2、3 章（流形曲面、同伦）——商空间构造与形变收缩显式同伦；
- 考前：过一遍本清单表格与判断题。
